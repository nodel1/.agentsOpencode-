...
echo [0/6] Verificando e instalando Git si es necesario...
where git >nul 2>nul
if %errorlevel% neq 0 (
    echo Git no esta instalado. Instalando...
    powershell -Command "winget install -e --id Git.Git --accept-package-agreements --accept-source-agreements --silent"
    if %errorlevel% neq 0 (
...
echo Git ya disponible.

echo [1/6] Configurando politica de ejecucion de PowerShell...
...
echo [2/6] Instalando dependencia global: agent-browser...
...
echo [3/6] Descargando binarios internos del navegador...
...
echo [4/6] Activando Git Hooks personalizados (.githooks)...
...
echo [5/6] Instalando RTK (Rust Token Killer) desde GitHub...
where rtk >nul 2>nul
if %errorlevel% neq 0 (
...
echo [6/6] Verificando configuracion final...
